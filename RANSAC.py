...
while (True):
    ...
    ax2 = axes[1]
    ax2.set_title("Savgol filtered Data with Linear Regression (RANSAC)")
    ...
    ax1.plot(data["eV"],data["Counts"])
    ax2.plot(savgolData[indName], savgolData[depName])
    ax2.plot(savgolData[indName], savgolData["filtCPS"])
    ax2.plot(line,regressor.predict(line))
